...
enum UserEventType
{
	...
	EVENT_BLACK,
	EVENT_HELP,
	EVENT_LINE, // Добавлено новое событие для рисования прямой
	EVENT_RECTANGLE, // Добавлено новое событие для рисования прямоугольника
	EVENT_FILLRECTANGLE
};
...
void MainWindow::OnCreate()
{
	...
	AddChild(new Button(" LINE ",EVENT_LINE), Point(450,700), Point(100,50));
	AddChild(new Button(" RECTANGLE", EVENT_RECTANGLE), Point(350,700), Point(100,50));
	AddChild(new Button(" FILLRECTANGLE", EVENT_FILLRECTANGLE), Point(450,700), Point(100,50));
}
...
void MainWindow::OnNotify(Window *child, uint32_t type, const Point &position)
{
	...
	else if(type == EVENT_LINE)
    {
        // Проверяем, что в векторе точек есть минимум 2 точки
        if (g_vector.size() >= 2)
        {
